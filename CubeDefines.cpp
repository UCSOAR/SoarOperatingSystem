...
/**
 * @brief Variadic log function, prints to UART and logs to external flash
 * @param str String to log with printf style formatting
 * @param ... Additional arguments to log, in same format as printf
 */
void cube_log(const char *str, ...)
{
#ifndef DISABLE_DEBUG
    // Try to take the VA list mutex
    if (Global::vaListMutex.Lock(DEBUG_TAKE_MAX_TIME_MS))
    {
        ...
        // ===== PART 1: Send to UART (same as SOAR_PRINT) =====
        Command cmd(DATA_COMMAND, (uint16_t)CUBE_TASK_COMMAND_SEND_DEBUG);
        cmd.CopyDataToCommand(str_buffer, buflen);
        CubeTask::Inst().GetEventQueue()->Send(cmd, false);

        // ===== PART 2: Log to external flash with timestamp =====
        // Prepend timestamp for context
        uint32_t timestamp = xTaskGetTickCount() * portTICK_PERIOD_MS;
        uint8_t timestamped_buf[DEBUG_PRINT_MAX_SIZE];
        int32_t ts_len = snprintf(reinterpret_cast<char *>(timestamped_buf),
                                  sizeof(timestamped_buf) - 1,
                                  "[%lu] %s",
                                  timestamp,
        ...
        if (ts_len > 0)
        {
            // Log to flash via LoggingService (length prefix is added automatically)
            LoggingService::LogText(timestamped_buf, static_cast<uint32_t>(ts_len));
        }
    }
    else
    {
